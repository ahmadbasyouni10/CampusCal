...
const Performance = ({ userId }) => {
  ...
  const groupTasksByMainTask = (tasks) => {
    const grouped = {};
    tasks.forEach((task) => {
      const mainTaskName = task.name.startsWith("Study for ")
        ? task.name.slice(10)
        : task.name;
  ...
  const renderTaskForm = (task) => {
    ...
    return (
      <div key={task.id} className="feedback-form">
        <h4>{task.name}</h4>
        ...
        <button
          ...
          disabled={
            !studyRatings[task.id] ||
            (isMainTask && !taskRatings[task.id]) ||
            !feelings[task.id]
          }
        >
          Submit
  ...
  return (
    <div className="performance-container">
      <div className="performance-header">
        ...
        <p>Track your progress, analyze your performance, and provide valuable feedback.</p>
      </div>
          ...
          {completedTasks.map((taskId, index) => {
            ...
            return (
              <div key={taskId} className="completed-task-item">
                <h4>{`Task ${index + 1}: ${task.name || "Unknown Task"}`}</h4>
      ...
      <h3 className="section-title">Feedback Forms</h3>
        ...
        {Object.entries(groupedTasks).map(([mainTaskName, taskData]) => {
          ...
          return (
            <div key={mainTaskName}>
              ...
              {studySessions.every((session) => completedTasks.includes(session.id)) &&
                mainTask &&
                !completedTasks.includes(mainTask.id) &&
                renderTaskForm(mainTask)}
            </div>
          );
        })}
      </div>
